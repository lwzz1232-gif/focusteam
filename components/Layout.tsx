...
export const Layout: React.FC<LayoutProps> = ({ children, user }) => {
  ...
  useEffect(() => {
    ...
    const unsub = firebaseService.onUserDocSnapshot(user.id, (docSnap) => {
      if (docSnap.exists()) {
  ...
  const showAdmin = (user?.role === 'admin' || user?.role === 'dev') && location.pathname === '/dashboard';
  const showFooter = location.pathname !== '/session';
  const showNotifications = user && location.pathname !== '/session';
