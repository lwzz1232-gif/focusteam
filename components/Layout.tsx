...
export const Layout: React.FC<LayoutProps> = ({ children, user, onLogout }) => {
  ...
  const isSessionScreen = location.pathname === '/session';
  const showFooter = !isSessionScreen;
  const showNotifications = user && !isSessionScreen;
                            ...
                            <div className="divide-y divide-white/5">
                              {notifications.map(note => (
                                <div
                                  ...
                                  className={`p-4 cursor-pointer transition-all duration-200 group relative ${!note.read ? 'bg-gradient-to-r from-blue-500/10 to-transparent hover:from-blue-500/20' : 'hover:bg-slate-800/50 opacity-70 hover:opacity-100'}`}
                                >
