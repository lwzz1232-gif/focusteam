...
import { collection, query, orderBy, getDocs, updateDoc, doc, limit, where, startAfter, DocumentData, QueryDocumentSnapshot, writeBatch, deleteDoc, addDoc } from 'firebase/firestore';
...
export const Admin: React.FC<AdminProps> = ({ onBack }) => {
  ...
  // Modal State (Replaces window.confirm/prompt)
  ...
  const fetchInitialData = async () => {
    ...
    try {
        if (activeTab === 'users') {
            ...
            setReports(reportsSnap.docs.map(d => ({ ...d.data(), id: d.id, timestamp: d.data().timestamp?.toMillis() || Date.now() } as Report)));
        }
  ...
  const openBanModal = (userId: string) => {
      setInputText('');
      setModal({ isOpen: true, type: 'BAN', data: userId, title: 'Ban User', message: 'Enter reason for 24h suspension:' });
  };
  ...
  return (
    <div className="flex-1 h-screen overflow-hidden flex flex-col bg-slate-950 relative font-inter">
      ...
      <div className="h-20 border-b border-white/5 flex items-center justify-between px-8 bg-slate-950/80 backdrop-blur-md z-10">
        ...
        <div className="flex items-center gap-3">
            ...
            <div className="bg-slate-900 border border-slate-700 rounded-lg px-3 py-1.5 flex items-center gap-2">
                ...
                <span className="text-xs font-mono text-slate-300">v3.0</span>
            </div>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-8 max-w-7xl mx-auto w-full">
                    ...
                    <div key={report.id} className="bg-slate-900/80 border border-red-500/20 rounded-xl p-5 shadow-lg relative overflow-hidden group hover:border-red-500/40 transition-colors">
                        ...
                        <div className="flex justify-between items-start mb-4">
                            <div>
                                ...
                                <div className="text-xs text-slate-400 mt-2">
                                    Reporter: <span className="font-mono text-white">{report.reporterId}</span>
                                </div>
                            </div>
                        ...
                        <div className="bg-black/30 p-3 rounded-lg border border-white/5 flex items-center justify-between">
                            <div>
                                ...
                                <div className="font-mono text-white text-sm flex items-center gap-2 cursor-pointer hover:text-blue-400" onClick={() => copyToClipboard(report.reportedId)}>
                                    {report.reportedId.substring(0,12)}... <Copy size={10} className="opacity-50"/>
                                </div>
                            </div>
        ...
        {/* --- SESSIONS TAB (Kept similar but cleaned up) --- */}
        {activeTab === 'sessions' && (
