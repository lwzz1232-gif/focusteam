...
import { SessionType, SessionDuration, SessionConfig, SessionMode, Partner } from '../types';
import { Briefcase, BookOpen, Code, Clock, Coffee, Play, FlaskConical, LogOut, ArrowDown, ShieldCheck } from 'lucide-react';
  ...
  const handleStart = async () => {
    ...
    if (config.duration === SessionDuration.TEST && (user?.role === 'admin' || user?.role === 'dev')) {
        ...
        try {
            ...
            dispatch({ type: 'START_TEST_SESSION', payload: { partner: botPartner, sessionId: testSessionId, config } });
        } catch (e) {
  ...
  return (
    <div className="flex-1 flex flex-col items-center justify-start p-4 max-w-5xl mx-auto w-full overflow-y-auto relative">
      ...
      <div id="live-lobby-section" className="w-full border-t border-white/5 pt-8 pb-8">
         <LiveRequests 
            currentUser={user} 
            onJoinSession={(partnerId, config) => dispatch({ type: 'START_MATCH', payload: config })}
         />
      </div>
