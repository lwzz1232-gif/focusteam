...
import { SessionPhase, SessionDuration, TodoItem } from '../types';
import { Button } from '../components/Button';
import { generateIcebreaker } from '../services/geminiService';
import { Mic, MicOff, Video, VideoOff, Sparkles, LogOut, User as UserIcon, MessageSquare, ListChecks, Flag, X, AlertTriangle, HeartCrack } from 'lucide-react';
...
export const LiveSession: React.FC = () => {
  const { user } = useAuth();
  const { partner, sessionConfig, sessionId, handleEndSession } = useSession();
  const navigate = useNavigate();

  useEffect(() => {
    if (!user || !partner || !sessionId || !sessionConfig) {
      navigate('/dashboard');
    }
  }, [user, partner, sessionId, sessionConfig, navigate]);

  const isTest = sessionConfig?.duration === SessionDuration.TEST;
  const [phase, setPhase] = useState<SessionPhase>(SessionPhase.ICEBREAKER);
  const [timeLeft, setTimeLeft] = useState(isTest ? 30 : (sessionConfig?.preTalkMinutes || 5) * 60);
  ...
  const calculatedFocusDuration = isTest ? 30 : (sessionConfig?.duration || 0) * 60;
  const isPomodoroMode = sessionConfig?.mode === 'POMODORO' || (sessionConfig as any)?.mode === 1;
  ...
  useEffect(() => {
    if (!sessionId || !user || !sessionConfig) return;
    const unsub = onSnapshot(doc(db, 'sessions', sessionId), (docSnap) => {
      ...
      if (data.phaseStartTime) {
        ...
        if (data.phase === SessionPhase.ICEBREAKER) totalDurationForPhase = isTest ? 30 : sessionConfig.preTalkMinutes * 60;
        else if (data.phase === SessionPhase.FOCUS) totalDurationForPhase = isTest ? 30 : sessionConfig.duration * 60;
        else if (data.phase === SessionPhase.DEBRIEF) totalDurationForPhase = isTest ? 30 : sessionConfig.postTalkMinutes * 60;
    ...
    return () => unsub();
  }, [sessionId, user, sessionConfig, isTest, handleEndSession]);
  ...
  return (
    <div className="absolute inset-0 bg-black overflow-hidden select-none font-sans">
      <style>{`
        @keyframes ripple-effect { 0% { transform: scale(0); opacity: 0.8; } 100% { transform: scale(4); opacity: 0; } }
        @keyframes messageFloatUp { 0% { transform: translateY(20px) scale(0.95); opacity: 0; } 10% { transform: translateY(0) scale(1); opacity: 1; } 90% { transform: translateY(-10px) scale(1); opacity: 1; } 100% { transform: translateY(-20px) scale(0.95); opacity: 0; } }
        .vignette { background: radial-gradient(circle, transparent 40%, rgba(0,0,0,0.9) 100%); }
      `}</style>

      {phase === SessionPhase.COMPLETED && (
        <div className="absolute inset-0 z-50">
          <SessionRecap user={user} partner={partner} duration={(Date.now() - startTime) / 60000} tasks={myTasks} onClose={handleEndSession} />
        </div>
      )}
      ...
      >
        {remoteStream ? (
          ...
          <div className="flex flex-col items-center justify-center h-full gap-4 bg-slate-900/50">
            <div className="relative"><div className="absolute inset-0 bg-blue-500 blur-xl opacity-20 animate-pulse"></div><UserIcon size={64} className="text-slate-700 relative z-10" /></div>
            <span className="text-slate-500 font-medium animate-pulse">Syncing...</span>
          </div>
        )}
        <div className="absolute inset-0 pointer-events-none overflow-hidden">
          {ripples.map(r => (<div key={r.id} className="absolute border border-white/50 rounded-full bg-white/10" style={{ left: r.x, top: r.y, width: '50px', height: '50px', marginLeft: '-25px', marginTop: '-25px', animation: 'ripple-effect 1s ease-out forwards' }} />))}
        </div>
      ...
      <div className={`absolute inset-0 pointer-events-none z-40 transition-opacity duration-500 ${showUI ? 'opacity-100' : 'opacity-0'}`}>
        <div className="absolute top-4 md:top-6 left-0 right-0 flex justify-center pointer-events-none px-14 md:px-0">
          <div className={`backdrop-blur-xl border rounded-full px-5 py-2 flex items-center gap-3 shadow-2xl transition-all duration-700 ${phase === SessionPhase.FOCUS ? 'bg-black/80 border-red-500/20 text-red-50' : 'bg-slate-900/80 border-slate-700'}`}>
            <span className={`text-[10px] font-bold uppercase tracking-widest ${phase === SessionPhase.FOCUS ? (isPomodoroBreak ? 'text-emerald-400' : 'text-red-400') : 'text-slate-400'}`}>
              {phase === SessionPhase.ICEBREAKER ? 'ICEBREAKER' : phase === SessionPhase.DEBRIEF ? 'DEBRIEF' : phaseLabel}
            </span>
        ...
        <div className="absolute top-4 md:top-6 left-4 md:left-6 pointer-events-auto">
          <button onClick={() => setIsReportOpen(true)} onMouseEnter={() => setIsInteracting(true)} onMouseLeave={() => setIsInteracting(false)} className="p-3 rounded-full bg-black/40 text-slate-400 hover:text-white border border-white/5 hover:bg-slate-800 transition-all"><Flag size={16} /></button>
        </div>
        <div className="absolute top-4 md:top-6 right-4 md:right-6 pointer-events-auto">
          <Button variant="danger" onClick={handleExitClick} onMouseEnter={() => setIsInteracting(true)} onMouseLeave={() => setIsInteracting(false)} className="py-2 px-3 text-xs bg-red-500/10 hover:bg-red-500/20 border-red-500/30 text-red-400 backdrop-blur-md"><LogOut size={14} className="mr-2" /> <span className="hidden md:inline">Exit</span></Button>
        </div>
        <div className="pointer-events-auto" onMouseEnter={() => setIsInteracting(true)} onMouseLeave={() => setIsInteracting(false)} onTouchStart={() => setIsInteracting(true)}>
        ...
        <div className="absolute bottom-8 left-0 right-0 px-4 flex flex-col md:flex-row items-center justify-center gap-4 pointer-events-auto">
          ...
          <div className="bg-black/80 backdrop-blur-md border border-white/10 rounded-full p-2 flex items-center gap-2 shadow-2xl order-2 md:order-none">
            {phase === SessionPhase.ICEBREAKER && (<Button onClick={async () => { setIsLoadingIcebreaker(true); setIcebreaker(await generateIcebreaker(partner.type)); setIsLoadingIcebreaker(false); }} variant="ghost" className="rounded-full w-10 h-10 p-0 text-yellow-400 hover:bg-yellow-400/10"><Sparkles size={18} className={isLoadingIcebreaker ? 'animate-spin' : ''} /></Button>)}
            <button onClick={() => { if (phase !== SessionPhase.FOCUS || isPomodoroBreak) { setMicEnabled(!micEnabled); setManualMicToggle(!micEnabled); } }} disabled={phase === SessionPhase.FOCUS && !isPomodoroBreak} className={`w-10 h-10 rounded-full flex items-center justify-center transition-all ${(phase === SessionPhase.FOCUS && !isPomodoroBreak) ? 'opacity-30 cursor-not-allowed' : micEnabled ? 'bg-white/10 hover:bg-white/20 text-white' : 'bg-red-500/20 text-red-400'}`}>{micEnabled && (phase !== SessionPhase.FOCUS || isPomodoroBreak) ? <Mic size={18} /> : <MicOff size={18} />}</button>
            <button onClick={() => setCamEnabled(!camEnabled)} className={`w-10 h-10 rounded-full flex items-center justify-center transition-all ${camEnabled ? 'bg-white/10 hover:bg-white/20 text-white' : 'bg-red-500/20 text-red-400'}`}>{camEnabled ? <Video size={18} /> : <VideoOff size={18} />}</button>
            <div className="w-px h-6 bg-white/10 mx-1"></div>
            <button onClick={() => { setIsChatOpen(!isChatOpen); setUnreadChatCount(0); }} className="w-10 h-10 rounded-full flex items-center justify-center hover:bg-white/10 text-slate-300 hover:text-white relative transition-colors"><MessageSquare size={18} />{unreadChatCount > 0 && <span className="absolute top-2 right-2 w-2 h-2 bg-blue-500 rounded-full ring-2 ring-black"></span>}</button>
            <button onClick={() => setIsTaskBoardOpen(!isTaskBoardOpen)} className={`w-10 h-10 rounded-full flex items-center justify-center transition-all ${isTaskBoardOpen ? 'bg-emerald-500 text-white' : 'hover:bg-white/10 text-slate-300 hover:text-white'}`}><ListChecks size={18} /></button>
          </div>
        </div>
        {icebreaker && (
            ...
            {isReportOpen && (
              <div className="bg-slate-900 border border-slate-700 rounded-2xl p-6 w-full max-w-sm shadow-2xl space-y-4 pointer-events-auto">
                ...
                <div className="space-y-2">
                  ...
                  <select value={reportReason} onChange={(e) => setReportReason(e.target.value)} className="w-full bg-slate-950 border border-slate-800 rounded-lg p-3 text-sm text-white focus:ring-1 focus:ring-blue-500 outline-none"><option>Inappropriate Behavior</option><option>Abusive Language</option><option>Spam / Commercial</option><option>Camera Off / Not Working</option><option>Other</option></select>
                </div>
                <div className="space-y-2">
