...
export const Negotiation: React.FC = () => {
  const { user } = useAuth();
  const { sessionConfig, partner, sessionId, handleNegotiationComplete, handleCancelMatch } = useSession();
  ...
  useEffect(() => {
    if (step === 'WAITING' && partnerMode && partnerPreTalk !== null && partnerPostTalk !== null && partnerDuration !== null && sessionConfig) {
        ...
        const agreedDuration = Math.min(sessionConfig.duration, partnerDuration);
  ...
  }, [step, partnerMode, partnerPreTalk, partnerPostTalk, partnerDuration, myMode, myPreTalk, myPostTalk, sessionConfig]);
  ...
  useEffect(() => {
    if (step === 'AGREED' && finalConfig) {
      const timer = setTimeout(() => handleNegotiationComplete(finalConfig), 2000);
      return () => clearTimeout(timer);
    }
  }, [step, finalConfig, handleNegotiationComplete]);
      ...
      <div className="relative z-10 w-full max-w-4xl p-6 mx-auto">
          ...
          <div className="w-full max-w-lg mx-auto animate-in fade-in slide-in-from-bottom-8 duration-500 bg-white/5 backdrop-blur-2xl rounded-3xl border border-white/10 p-1 shadow-2xl">
            ...
            <div className="p-8">
                ...
                <div className="space-y-8">
                   <div className="grid grid-cols-2 gap-4">
                        ...
                        <button
                          ...
                          disabled={sessionConfig.duration < 60}
                          className={`p-4 rounded-2xl border text-sm font-medium flex flex-col items-center gap-2 transition-all duration-300 ${
                             myMode === SessionMode.POMODORO
                             ? 'bg-blue-600/20 border-blue-500/50 text-white shadow-[0_0_20px_rgba(37,99,235,0.2)]'
                             : 'bg-white/5 border-white/5 text-slate-400 hover:bg-white/10'
                          } ${sessionConfig.duration < 60 ? 'opacity-30 cursor-not-allowed' : ''}`}
                        >
                ...
                <div className="flex gap-4 mt-8">
                   <button onClick={handleCancelMatch} className="px-6 py-4 rounded-xl text-slate-400 hover:text-white hover:bg-white/5 text-sm font-medium transition-colors">
                      Skip
                   </button>
        ...
        {step === 'REVIEW' && (
           <div className="w-full max-w-3xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-500">
               ...
               <div className="grid grid-cols-2 gap-1 bg-white/5 backdrop-blur-xl rounded-3xl p-1 border border-white/10">
                  <div className="bg-black/20 rounded-l-2xl p-6 flex flex-col gap-4 text-center">
                      ...
                      <div className="space-y-2">
                          ...
                          <div className={`text-xl font-light ${sessionConfig.duration !== partnerDuration ? 'text-amber-300' : 'text-white'}`}>{sessionConfig.duration}m</div>
                      </div>
                  ...
                  <div className="bg-black/20 rounded-r-2xl p-6 flex flex-col gap-4 text-center border-l border-white/5">
                      ...
                      <div className="space-y-2">
                          <div className="text-xs text-slate-500 uppercase">Duration</div>
                          <div className={`text-xl font-light ${sessionConfig.duration !== partnerDuration ? 'text-amber-300' : 'text-white'}`}>{partnerDuration}m</div>
                      </div>
                      <div className="space-y-1">
                          <div className="text-xs text-slate-500 uppercase">Mode</div>
               ...
               {partnerDuration && partnerDuration !== sessionConfig.duration && (
